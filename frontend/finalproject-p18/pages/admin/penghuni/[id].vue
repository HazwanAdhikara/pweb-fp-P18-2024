<template>
  <div class="p-6 bg-[#f8f9fa] min-h-screen">
    ...
    <div v-else>
      ...
      <div v-if="invoiceHistory.length === 0" class="text-gray-500">No invoice history available.</div>
      <div v-else>
        <div
          v-for="(invoice, index) in invoiceHistory"
          :key="index"
          class="p-4 bg-white rounded-lg shadow mb-4"
        >
          <p><strong>Bill:</strong> Rp {{ invoice.bill.toLocaleString() }}</p>
          <p><strong>Payment Date:</strong> {{ formatDate(invoice.created_at) }}</p>
    ...
    const fetchUserDetails = async () => {
      try {
        ...
        const response = await fetch(
          `http://localhost:4000/admin/penghuni/${route.params.id}`,
          {
            method: "GET",
            headers: {
